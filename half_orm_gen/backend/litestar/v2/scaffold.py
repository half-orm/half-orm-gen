...
GET /auth/login?peer=<name>&return_to=<path>
  (This peer is the REQUESTER.) Redirect the browser to the named trusted
...
def _sign_local_token(user_id: str) -> str:
    payload = {'sub': user_id, 'roles': ['connected'], 'iat': int(time.time())}
...
def make_federation_handlers(model) -> list:
    ...
    async def auth_login(
        ...
        rows = await identity.peer()(name=peer, trusted=True).ho_aselect('id', 'url')
        if not rows:
    ...
    async def federation_callback(token: str, csrf_state: str) -> Redirect:
        ...
        local_token = _sign_local_token(user_id)
...
def scaffold_api_dir(
    ...
    if not env_file.exists():
        if federation:
            ...
            print(f'  NOTE: set HO_PEER_URL and HO_FRONTEND_URL in {env_file} to this project\'s public base URLs')
        else:
